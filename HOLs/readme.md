...
1. Ok, so seeing the debug messages is interesting, but it would be cooler if we could see the temperatures displayed on the LCD panel. To do that, from the **UPM_Sensors**, drag the "**Grove RGB LCD**" on the visual editor as shown below, connect it to the output of the "**Data Conversion**" node, and configure it as follows:
...
1. In the "**Add New Packages**" window, in the search box, type "**cloud-azure**", then click the "**Install**" button next to the "**packagegroup-cloud-azure**" package.  Again, this takes a few minutes so be patient:
    ...
    > **Note**: You can see what all is installed with the packagegroup-cloud-azure package here: [link](https://github.com/intel-iot-devkit/meta-iot-cloud/blob/master/recipes-core/packagegroups/packagegroup-cloud-azure_0.9.bb) <br/>
    Basically it is all of the node.js packages for the various Azure IoT Hub sdks.  It also includes a Node-RED node for working with Azure IoT Hubs ([link](https://www.npmjs.com/package/node-red-contrib-azureiothubnode)).  
...
1. Drag the node onto the Node-RED visual, connect it to the "**json**" node's output as shown below, configure it as follows, and click "**OK**"
...
In this task, we'll walk through publishing a pre-created Power BI report into a Power BI collection and Workspace in your Azure Subscription.  You can learn more about Power BI Embedded here: [https://azure.microsoft.com/en-us/services/power-bi-embedded/](https://azure.microsoft.com/en-us/services/power-bi-embedded/) 

1. You **DO NOT NEED** to edit the report provided, however, if you would like to see how it was authored, and your are on a Windows system, you can download "**Power BI Desktop**" from [https://powerbi.microsoft.com/en-us/desktop/](https://powerbi.microsoft.com/en-us/desktop/).  Once you have downloaded it, you can open the "**HOLs\PowerBI\TemperatureChart.pbix**" file to view how it was designed.  **REGARDLESS, DO NOT MAKE CHANGES TO THE REPORT AT THIS TIME!**
...
1. Once the new Power BI Embedded Workspace Collection is created, click the "**Access keys**" button along the right of it's blade:
